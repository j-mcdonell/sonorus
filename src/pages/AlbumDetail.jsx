import React, { useMemo, useState, useContext } from 'react'; // FIX: Added useContext
import { supabase } from '@/api/supabaseClient';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { Link } from 'react-router-dom';
import { createPageUrl } from '@/utils';
import { motion, AnimatePresence } from 'framer-motion';
import { AuthContext } from '@/lib/AuthContext'; // FIX: Import AuthContext
import { 
...
export default function AlbumDetail() {
    ...
    mutationFn: async (reviewData) => {
      ...
      const { data, error } = await supabase
        ...
        .insert([{ 
          ...reviewData, 
          album_id: albumId,
          created_by: user.email // FIX: Add the user's email
        }]);
  ...
  const scores = useMemo(() => {
    ...
    return {
      overall: calcAvg(reviews),
      critic: calcAvg(criticReviews),
  ...
  if (!album) {
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center text-white p-4">
        ...
        <Link to={createPageUrl('Home')} className="text-violet-400 hover:underline">
          Go back home
        </Link>
      </div>
    );
      ...
      <div className="relative">
        ...
        <div className="relative max-w-6xl mx-auto px-4 pt-8 pb-12">
          ...
          <div className="flex flex-col md:flex-row gap-8">
            ...
            >
              <img
                src={album.cover_url || 'https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?w=400'}
                alt={album.title}
                className="w-64 h-64 md:w-72 md:h-72 rounded-2xl shadow-2xl shadow-black/50 object-cover mx-auto md:mx-0"
              />
            </motion.div>
            ...
            <div className="flex-1 text-center md:text-left">
              {album.genre && (
                <div className="mb-3">
                  <GenreTag genre={album.genre} size="md" />
                </div>
              )}
              <motion.h1 
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-sm text-zinc-500 mb-6">
                {album.release_year && (
                  <span className="flex items-center gap-1.5">
                    <Calendar className="w-4 h-4" /> {album.release_year}
                  </span>
                )}
                {tracklistArray.length > 0 && (
                  <span className="flex items-center gap-1.5">
                    <ListMusic className="w-4 h-4" /> {tracklistArray.length} tracks
                  </span>
                )}
                <span className="flex items-center gap-1.5">
                  <PenLine className="w-4 h-4" /> {reviews.length} reviews
                </span>
              </div>

              <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 mb-6">
                <div className="text-center">
                  <ScoreBadge score={scores.overall} size="xl" />
                ...
                {scores.critic !== null && (
                  <div className="text-center">
                    <div className="flex items-center gap-2 justify-center mb-1">
                      ...
                      <span className="text-sm text-zinc-400">Critics</span>
                    </div>
                    <ScoreBadge score={scores.critic} size="md" />
                  </div>
                )}
                {scores.user !== null && (
                  <div className="text-center">
                    ...
                    <ScoreBadge score={scores.user} size="md" />
                  </div>
                )}
              </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            {album.description && (
              <div className="bg-zinc-900/50 rounded-2xl p-6 border border-zinc-800/50">
                ...
                <p className="text-sm text-zinc-400 leading-relaxed">{album.description}</p>
              </div>
            )}
            ...
            {tracklistArray.length > 0 && (
              <div className="bg-zinc-900/50 rounded-2xl p-6 border border-zinc-800/50">
                <h3 className="font-semibold mb-4 flex items-center gap-2">
                  <ListMusic className="w-4 h-4 text-violet-400" /> Tracklist
                </h3>
                <ul className="space-y-2">
                  {visibleTracks.map((track, index) => (
                    <li 
                      key={index}
                      className="flex items-center gap-3 text-sm py-2 border-b border-zinc-800/50 last:border-0"
                    >
                      <span className="w-6 h-6 flex items-center justify-center rounded-full bg-zinc-800 text-xs text-zinc-400">
                        {index + 1}
                      </span>
                ...
                {tracklistArray.length > 5 && (
                  <button
                    ...
                    className="w-full mt-4 flex items-center justify-center gap-1 text-sm text-violet-400 hover:text-violet-300"
                  >
                    {showAllTracks ? (
                      <>Show less <ChevronUp className="w-4 h-4" /></>
                    ) : (
                      <>Show all {tracklistArray.length} tracks <ChevronDown className="w-4 h-4" /></>
                    )}
                  </button>
                )}
              </div>
              ...
              <div className="p-6">
                {reviewsLoading ? (
                  ...
                  <div className="text-center py-12">
                    <Star className="w-12 h-12 mx-auto text-zinc-700 mb-4" />
                    <h4 className="text-lg font-medium text-zinc-400">No reviews yet</h4>
